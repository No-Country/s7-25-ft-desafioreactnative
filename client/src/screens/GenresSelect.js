...
export default function ForgotPasword() {
  ...
  useEffect(() => {
    ...
    axios.get(`${BaseURL}/api/v1/genres`)
    .then((response) => {
      setsongs(response.data.genres);
    })
    .catch((error) => {
      console.log(error);
    });
  ...
  return (
    ...
    <FlatList contentContainerStyle={{alignItems:'center',}} style={{marginTop:height*0.02}} data={songs} renderItem={({item}) => <Item title={item.name} />} keyExtractor={item => item.id} numColumns={2} />
